...
public class UnitTest1
{
    ...
    {
        var user = new Solana.Unity.KeyStore.SolanaKeyStoreService().RestoreKeystoreFromFile(env["wallet"]);
    ...
    private async Task<string> GetNameForMint(PublicKey mint)
    {
        ...
        var acc = await rpc.GetAccountInfoAsync(Lib.DerivePda(metaId, "metadata", metaId, mint).Key, Solana.Unity.Rpc.Types.Commitment.Confirmed);
        var name = Encoding.UTF8.GetString(Convert.FromBase64String(acc.Result.Value.Data[0]).Skip(65).Take(36).ToArray()).Trim('\0').Trim();
        return name;

    }

    private async Task<Solana.Unity.Rpc.Core.Http.RequestResult<string>> PrepareAndSend(TransactionInstruction instruction, Account signer){
